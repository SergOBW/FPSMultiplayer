...
public class bl_Gun : bl_GunBase
{
    ...
    {
        bulletsLeft = BulletsPerMagazine; // load gun on startup
    ...
    public override void OnUpdate()
    {
        ...
        {
            spread += (PlayerReferences.movementBehaviour.GetPlayerState() == PlayerState.Crouching) ? spreadPerSecond * 0.5f : spreadPerSecond; // gun is less accurate with the trigger held down
        }
        else
        {
    ...
    void SingleFire()
    {
        if (PlayerReferences.movementBehaviour.GetPlayerState() == PlayerState.Running && Time.time - lastShotTime > (Info.FireRate * 2) || pendingFirstShot)
        {
    ...
    void LoopFire()
    {
        ...
        if (PlayerReferences.movementBehaviour.GetPlayerState() == PlayerState.Running && Time.time - lastShotTime > (Info.FireRate * 5) || pendingFirstShot)
        {
            if (pendingFirstShot) return;
    ...
    void FireOneShot()
    {
        ...
        bl_CrosshairBase.Instance.OnFire();
        PlayerNetwork.ReplicateFire(WeaponType, instanceData.ProjectedHitPoint, BulletSettings.Inaccuracity);
    ...
    void DetermineUpperState()
    {
        ...
        else if (controller.GetPlayerState() == PlayerState.Running && !isReloading && !isFiring && !isAiming)
        {
    ...
    {
        get { return PlayerReferences.movementBehaviour.GetPlayerState() == PlayerState.Crouching ? spreadMinMax.x * 0.5f : spreadMinMax.x; }
    }
    ...
    bool FireWhileRun
    {
        get
        {
            ...
            if (controller.GetPlayerState() != PlayerState.Running)
            {
                return true;
            }
            else
    ...
    public MovementBehaviour controller => PlayerReferences.movementBehaviour;
