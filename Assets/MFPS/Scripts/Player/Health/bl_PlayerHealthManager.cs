...
public class bl_PlayerHealthManager : bl_PlayerHealthManagerBase
{
    ...
    {
        if (!bl_GameData.Instance.allowFallDamage || isProtectionEnable || playerReferences.movementBehaviour.GetPlayerState() == PlayerState.InVehicle || bl_GameManager.Instance.GameMatchState == MatchState.Waiting)
            return;
    ...
    void SyncDamage(int damage, string killer, DamageCause cause, Vector3 hitDirection, bool isHeatShot, int weaponID, Player m_sender)
    {
        ...
        if (DamageEnabled)
        {
            if (currentHealth >= 0)
            {
                if (isMine)
                {
                    damageAlphaValue += (damage + ((maxHealth - currentHealth) * (Time.deltaTime * Mathf.PI))) * 0.2f;
	...
	void Die(string killer, bool isHeadshot, DamageCause cause, int gunID, Vector3 hitPos, Player sender)
    {
        ...
        else//when is local player who dies
        {
            ...
            if (bl_GameData.Instance.DropGunOnDeath)
            {
                ...
                {
                    Debug.LogError("There is no gun manager attached on player");
                }
            }
