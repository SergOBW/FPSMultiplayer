...
namespace MFPS.Addon.Customizer
{
    ...
    public class bl_Customizer : MonoBehaviour
    {
        ...
        //Private Vars
        public int[] AttachmentsIDs = new int[] { 0, 0, 0, 0, 0, 0 };
        ...
        public void Randomize()
        {
            ...
            AttachmentsIDs[4] = Random.Range(0, Attachments.Lasers.Count);
            int pci = AttachmentsIDs[5];
            AttachmentsIDs[5] = availableCamos[Random.Range(0, availableCamos.Count)];
            if (AttachmentsIDs[5] != pci)
            {
                StopCoroutine(nameof(CamoChangeEffect));
                StartCoroutine(nameof(CamoChangeEffect), AttachmentsIDs[5]);
            }
        }
        ...
        void DrawInfo()
        {
            ...
            if (Button.LaserButton != null)
            {
                Vector3 pointCylinder = m_Camera.WorldToScreenPoint(Button.LaserButton.transform.position);
                foreach (CustomizerModelInfo IdLaser in Attachments.Lasers)
                {
                    if (IdLaser.ID == AttachmentsIDs[4])
                    {
                        //position is in screen?
                        if (rect.Contains(pointCylinder))
                        {
                            GUI.Label(new Rect(pointCylinder.x - IdLaser.Info.Description.Length, (Screen.height - pointCylinder.y) + 15, 250, 75), "<size=10>" + IdLaser.Info.Description + "</size>");
                        }
                    }
                }
            }
        ...
        void RendererModule()
        {
            ...
            foreach (CustomizerModelInfo IdLaser in Attachments.Lasers)
            {
                if (IdLaser.Model == null) continue;
                if (IdLaser.ID == AttachmentsIDs[4])
                {
                    IdLaser.Model.SetActive(true);
                }
                else
                {
                    IdLaser.Model.SetActive(false);
                }
            }
        ...
        IEnumerator ChangeButtonPosition(bool open)
        {
            ...
            if (Positions.LaserRoot != null)
                v[4] = (open) ? Positions.LaserRoot.transform.position : Positions.LaserRoot.transform.localPosition;
            while (d < 1)
            {
                if (open)
                {
                    ...
                    if (Positions.LaserRoot != null && Positions.LaserPosition != null)
                        Positions.LaserRoot.transform.position = Vector3.Lerp(v[4], Positions.LaserPosition.position, t);
                }
                else
                {
                    ...
                    if (Positions.LaserRoot != null)
                        Positions.LaserRoot.transform.localPosition = Vector3.Lerp(v[4], Positions.defaultPositions[4], t);
                }
                yield return null;
            }
        }
        ...
        void DetectButton()
        {
            if (Customize && Button.ActiveButton != null)
            {
                if (Input.GetMouseButtonDown(0))
                {
                    ...
                    else if (Button.ActiveButton.m_Type == bl_AttachType.Laser)
                    {
                        Manager.ChangeAttachWindow(Attachments.Lasers, bl_AttachType.Laser);
                    }
                }
            }
        }
        ...
        public void BuildAttachments()
        {
            ...
            for (int i = 0; i < info.Attachments.Lasers.Count; i++)
            {
                Attachments.Lasers.Add(new CustomizerModelInfo());
                Attachments.Lasers[i].SetInfo(info.Attachments.Lasers[i]);
            }
        }
        ...
        public void RefreshAttachments()
        {
            ...
            for (int i = 0; i < info.Attachments.Lasers.Count; i++)
            {
                if (Attachments.Lasers.Count - 1 < i)
                {
                    Attachments.Lasers.Add(new CustomizerModelInfo());
                }
                Attachments.Lasers[i].SetInfo(info.Attachments.Lasers[i]);
        ...
        void OnDrawGizmos()
        {
            ...
            if (Positions.LaserRoot != null)
            {
                Gizmos.DrawLine(Positions.LaserRoot.transform.position, Positions.LaserPosition.position);
                Gizmos.DrawWireSphere(Positions.LaserPosition.position, 0.07f);
            }
            ...
            if (Positions.LaserPosition != null)
            {
                Gizmos.DrawSphere(Positions.LaserPosition.position, 0.07f);
            }
        }
#endif
    }
